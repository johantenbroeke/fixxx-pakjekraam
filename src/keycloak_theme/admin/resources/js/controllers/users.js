module.controller('UserRoleMappingCtrl', function(
    $scope,
    $http,
    realm,
    user,
    ...
    $scope.deleteRealmRole = function() {
        ...
        $http
            .delete(authUrl + '/admin/realms/' + realm.realm + '/users/' + user.id + '/role-mappings/realm', {
        ...
        $http
            .delete(
                authUrl +
                    ...
                    '/role-mappings/clients/' +
                    $scope.targetClient.id,
                { data: $scope.clientRolesToRemove, headers: { 'content-type': 'application/json' } },
            )
            .then(function() {
                $scope.clientMappings = ClientRoleMapping.query({
                    realm: realm.realm,
...
module.controller('UserSessionsCtrl', function(
    $scope,
    realm,
    user,
    sessions,
...
module.controller('UserFederatedIdentityCtrl', function(
    $scope,
    $location,
    realm,
    user,
    ...
    $scope.removeProviderLink = function(providerLink) {
        ...
        Dialog.confirmDelete(providerLink.identityProvider, 'Identity Provider Link', function() {
            UserFederatedIdentity.remove(
                { realm: realm.realm, user: user.id, provider: providerLink.identityProvider },
                function() {
...
module.controller('UserFederatedIdentityAddCtrl', function(
    $scope,
    $location,
    realm,
    user,
...
module.controller('UserListCtrl', function(
    $scope,
    realm,
    User,
    UserSearchState,
...
module.controller('UserDetailCtrl', function(
    $scope,
    realm,
    user,
    BruteForceUser,
            ...
            if ($scope.access.viewRealm) {
                Components.get({ realm: realm.realm, componentId: user.federationLink }, function(link) {
                    ...
                    $scope.federationLink =
                        '#/realms/' + realm.realm + '/user-storage/providers/' + link.providerId + '/' + link.id;
                });
            } else {
                // KEYCLOAK-4328
                UserStorageOperations.simpleName.get({ realm: realm.realm, componentId: user.federationLink }, function(
                    link,
                ) {
                    $scope.federationLinkName = link.name;
...
module.controller('UserCredentialsCtrl', function(
    $scope,
    realm,
    user,
    $route,
        ...
        Dialog.confirm(
            ...
            function() {
                UserCredentials.resetPassword(
                    ...
                    { type: 'password', value: $scope.password, temporary: $scope.temporaryPassword },
                    function() {
    ...
    $scope.sendExecuteActionsEmail = function() {
        ...
        Dialog.confirm('Send Email', 'Are you sure you want to send email to user?', function() {
            UserExecuteActionsEmail.update(
                { realm: realm.realm, userId: user.id, lifespan: $scope.emailActionsTimeout.toSeconds() },
                $scope.emailActions,
                function() {
...
module.controller('UserFederationCtrl', function(
    $scope,
    $location,
    $route,
    realm,
    ...
    Components.query(
        { realm: realm.realm, parent: realm.id, type: 'org.keycloak.storage.UserStorageProvider' },
        function(data) {
...
module.controller('GenericUserStorageCtrl', function(
    $scope,
    $location,
    Notifications,
    $route,
    ...
    function initUserStorageSettings() {
        if ($scope.create) {
            ...
            $scope.changedSyncEnabled =
                instance.config['changedSyncPeriod'] && instance.config['changedSyncPeriod'][0] > 0;
            if (providerFactory.metadata.synchronizable) {
    ...
    function triggerSync(action) {
        UserStorageOperations.sync.save(
            { action: action, realm: $scope.realm.realm, componentId: $scope.instance.id },
            {},
            function(syncResult) {
                $route.reload();
...
module.controller('UserGroupMembershipCtrl', function(
    $scope,
    $q,
    realm,
    user,
    ...
    $scope.$watch('currentPage', function(newValue, oldValue) {
        if (newValue !== oldValue) {
            refreshAvailableGroups($scope.searchCriteria).then(function() {
                refreshUserGroupMembership($scope.searchCriteriaMembership);
            });
        }
    });
    ...
    $scope.joinGroup = function() {
        ...
        UserGroupMapping.update(
            { realm: realm.realm, userId: user.id, groupId: $scope.tree.currentNode.id },
            function() {
    ...
    $scope.leaveGroup = function() {
        ...
        UserGroupMapping.remove(
            { realm: realm.realm, userId: user.id, groupId: $scope.membershipTree.currentNode.id },
            function() {
...
module.controller('LDAPUserStorageCtrl', function(
    $scope,
    $location,
    Notifications,
    $route,
    ...
    if (serverInfo.profileInfo.name == 'community') {
        ...
        $scope.ldapVendors = [{ id: 'ad', name: 'Active Directory' }, { id: 'rhds', name: 'Red Hat Directory Server' }];
    }

    $scope.authTypes = [{ id: 'none', name: 'none' }, { id: 'simple', name: 'simple' }];

    $scope.searchScopes = [{ id: '1', name: 'One Level' }, { id: '2', name: 'Subtree' }];
...
module.controller('LDAPMapperListCtrl', function(
    $scope,
    $location,
    Notifications,
    $route,
...
module.controller('LDAPMapperCtrl', function(
    $scope,
    $route,
    realm,
    provider,
    ...
    $scope.save = function() {
        Components.update({ realm: realm.realm, componentId: mapper.id }, $scope.mapper, function() {
            $route.reload();
    ...
    function triggerMapperSync(direction) {
        LDAPMapperSync.save(
            { direction: direction, realm: realm.realm, parentId: provider.id, mapperId: $scope.mapper.id },
            {},
            function(syncResult) {
...
module.controller('LDAPMapperCreateCtrl', function(
    $scope,
    realm,
    provider,
    mapperTypes,
